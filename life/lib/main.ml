...
let alive w i j rule =
  ...
  let cpl = match rule with
    | Rule (s, b) -> (s, b) in

  if cell then (* cell is alive *)
    (* cell survives if the number of alive neighbors is in the 'survive' list *)
    List.mem alive_nb (fst cpl)

  else (* cell is dead *)
    (* cell is born if the number of alive neighbors is in the 'born' list *)
    List.mem alive_nb (snd cpl)
